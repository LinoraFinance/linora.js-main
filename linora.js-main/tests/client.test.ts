...
import { linoraClient } from '../src/client';
...
describe('linoraClient', () => {
  describe('createFromEthSigner', () => {
    test('should create client from Ethereum signer', async () => {
      ...
      const client = await linoraClient.createFromEthSigner({
        config,
        signer,
      });

      expect(client).toBeInstanceOf(linoraClient);
    ...
    test('should throw error if provider creation fails', async () => {
      ...
      jest
        .spyOn(linoraClient as any, 'createFromEthSigner')
        .mockRejectedValueOnce(
          new Error('Failed to create authenticated provider'),
        );

      await expect(
        linoraClient.createFromEthSigner({ config, signer }),
  ...
  describe('createFromStarknetAccount', () => {
    test('should create client from Starknet account', async () => {
      ...
      const config = configFactory();
      const client = await linoraClient.createFromStarknetAccount({
        config,
        account: snAccount,
