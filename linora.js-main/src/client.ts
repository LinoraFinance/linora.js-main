...
import type { linoraConfig } from './config.js';
...
/**
 * High-level linora client with clean interface
 */
export class linoraClient {
  private readonly provider: DefaultProvider;
  private readonly config: linoraConfig;
  ...
  /**
   * Create a linora client from an Ethereum signer
   */
  static async createFromEthSigner({
    config,
    signer,
  }: CreateClientFromEthSignerParams): Promise<linoraClient> {
    ...
    return new linoraClient(config, provider, account);
  }

  /**
   * Create a linora client from a Starknet account
   */
  static async createFromStarknetAccount({
    ...
    starknetProvider,
    rpcUrl,
  }: CreateClientFromStarknetAccountParams): Promise<linoraClient> {
    // Derive credentials internally (privateKey never exposed)
    ...
    // Get the account from provider
    const linoraAccount = provider.getAccount();
    if (linoraAccount == null) {
      throw new Error('Failed to create authenticated provider');
    }

    return new linoraClient(config, provider, linoraAccount);
  }
