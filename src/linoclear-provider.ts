...
/**
 * Stateless RPC provider that supports linora authentication
...
export class DefaultProvider extends AuthenticatedRpcProvider {
  ...
  constructor(
    ...
    super(config.linoraFullNodeRpcUrl, config.linoraChainId);
    this.nodeUrl = config.linoraFullNodeRpcUrl;
  ...
  override async callContract(
    ...
    const authHeaders = {
      ...
      'linora-STARKNET-ACCOUNT': signature.account,
      'linora-STARKNET-SIGNATURE': JSON.stringify(signature.signature),
      'linora-STARKNET-SIGNATURE-TIMESTAMP': signature.timestamp,
      'linora-STARKNET-SIGNATURE-VERSION': '1.0.0',
    };
