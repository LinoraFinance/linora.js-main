...
describe('AuthenticatedRpcProvider', () => {
  test('should create provider with correct chain ID', () => {
    ...
    const provider = new AuthenticatedRpcProvider(
      config.linoraFullNodeRpcUrl,
      config.linoraChainId,
    );

    expect(provider.getChainIdString()).toBe(config.linoraChainId);
  });
...
describe('DefaultProvider', () => {
  ...
  describe('callContract', () => {
    ...
    test('should call contract with authentication when account exists', async () => {
      ...
      expect(headers['linora-STARKNET-ACCOUNT']).toBeDefined();
      expect(headers['linora-STARKNET-SIGNATURE']).toBeDefined();
      expect(headers['linora-STARKNET-SIGNATURE-TIMESTAMP']).toBeDefined();
      expect(headers['linora-STARKNET-SIGNATURE-VERSION']).toBe('1.0.0');
